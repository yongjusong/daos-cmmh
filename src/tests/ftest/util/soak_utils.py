...
from run_utils import RunException, daos_env_str, run_local, run_remote
...
def start_dfuse(self, pool, container, name=None, job_spec=None):
    ...
    # Carry forward any DAOS-related envs
    dfuse_env_dict = os.environ.copy()
    dfuse_env_dict.update({
        "D_LOG_FILE_APPEND_PID": "1",
        "D_LOG_MASK": "ERR",
        "D_LOG_FILE": dfuselog
    })
    dfuse_env = daos_env_str(dfuse_env_dict).rstrip(';')
...
def stop_dfuse(dfuse, vol=False):
    ...
    # Carry forward any DAOS-related envs
    dfuse_env = daos_env_str(os.environ).rstrip(';')

    dfuse_stop_cmds.extend([
        'clush -S -w $HOSTLIST "{}; fusermount3 -uz {}"'.format(dfuse_env, dfuse.mount_dir.value),
        'clush -S -w $HOSTLIST "{}; rm -rf {}"'.format(dfuse_env, dfuse.mount_dir.value)])
